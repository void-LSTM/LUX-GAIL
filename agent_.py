...
class Agent:
    def __init__(self, policy_net: PolicyNetwork, state_network,device, research_th: float = 0.0, research_turn: int = 10):
    ...
    def __call__(self, observation, configuration) -> List[str]:
        ...
        feature=self.state_network(state, masking)
        (actions, action_probs, _), (citytile_actions, citytile_action_probs, _) = self.policy_net.act(feature, target)
        ...
        return result_actions,state,masking
